...
function Primus(url, options) {
  ...
  primus.timers = new Tick();                   // Contains all our timers.
  ...
  //
  // Setup our reconnection functionality
  //
  primus.recovery = new Recovery(this, options.reconnect);
...
Primus.prototype.initialise = function initialise(options) {
  ...
  primus.on('incoming::error', function error(e) {
    ...
    //
    if (primus.recovery.active()) {
      return primus.recovery.failed(err);
    }
    ...
    //
    if (connect) {
      if (~primus.options.strategy.indexOf('timeout')) primus.recovery.reconnect();
      else primus.end();
    }
  });
  ...
  primus.on('incoming::end', function end() {
    ...
    if (readyState !== Primus.OPEN) {
      return primus.recovery.active() ? primus.recovery.failed() : false;
    }
